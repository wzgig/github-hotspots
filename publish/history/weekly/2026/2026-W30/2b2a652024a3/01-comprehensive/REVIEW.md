...
先看结论：
01｜mattpocock/skills：公开信息不足，暂不能准确说明 skills 的具体用途
02｜tirth8205/code-review-graph：想要检查代码变更并整理审查意见，可以用 code-review-graph；它能帮助团队发现问题和统一修改建议
03｜stablyai/orca：主要任务是集中运行和管理多个编程 Agent；orca 还能让开发任务可以并行推进
04｜bojieli/ai-agent-book：用 ai-agent-book 可以把外部工具和数据接入支持 MCP 的模型应用，还会用统一协议建立工具调用连接
05｜rohitg00/ai-engineering-from-scratch：这类任务可以交给 ai-engineering-from-scratch：把外部工具和数据接入支持 MCP 的模型应用，并用统一协议建立工具调用连接
06｜koala73/worldmonitor：当你需要把外部工具和数据接入支持 MCP 的模型应用，worldmonitor 可以接手，同时用统一协议建立工具调用连接
07｜diegosouzapw/OmniRoute：如果工作流里要统一接入并路由多个 AI 模型服务，可以交给 OmniRoute，同时把模型选择和供应商配置集中管理
...
- 定位：公开信息不足，暂不能准确说明 skills 的具体用途
- Star：188,544
- Fork：16,197
- 本期信号：+11,961 Star（snapshot）
- 能力：需要查看仓库 README 确认主要任务；需要核对实际输入、输出和使用方式；需要人工确认适用场景后再发布

### 02｜tirth8205/code-review-graph

- 仓库：https://github.com/tirth8205/code-review-graph
- 定位：想要检查代码变更并整理审查意见，可以用 code-review-graph；它能帮助团队发现问题和统一修改建议
- Star：26,441
- Fork：2,471
- 本期信号：+6,296 Star（snapshot）
- 能力：逐项检查提交中的代码变更；标出潜在缺陷和需要确认的实现；把审查意见整理成可跟进的建议

### 03｜stablyai/orca

- 仓库：https://github.com/stablyai/orca
- 定位：主要任务是集中运行和管理多个编程 Agent；orca 还能让开发任务可以并行推进
- Star：29,144
- Fork：2,068
- 本期信号：+7,330 Star（snapshot）
- 能力：在同一界面启动多个编程 Agent；把任务拆给不同 Agent 并行执行；集中查看各个 Agent 的执行状态

### 04｜bojieli/ai-agent-book

- 仓库：https://github.com/bojieli/ai-agent-book
- 定位：用 ai-agent-book 可以把外部工具和数据接入支持 MCP 的模型应用，还会用统一协议建立工具调用连接
- Star：19,975
- Fork：1,990
- 本期信号：+16,579 Star（trending）
- 能力：为模型应用提供标准化工具接口；连接外部数据源和可调用工具；把工具能力暴露给兼容 MCP 的客户端

### 05｜rohitg00/ai-engineering-from-scratch

- 仓库：https://github.com/rohitg00/ai-engineering-from-scratch
- 定位：这类任务可以交给 ai-engineering-from-scratch：把外部工具和数据接入支持 MCP 的模型应用，并用统一协议建立工具调用连接
- Star：43,526
- Fork：7,292
...
- 定位：当你需要把外部工具和数据接入支持 MCP 的模型应用，worldmonitor 可以接手，同时用统一协议建立工具调用连接
- Star：74,270
- Fork：11,151
...
- 定位：如果工作流里要统一接入并路由多个 AI 模型服务，可以交给 OmniRoute，同时把模型选择和供应商配置集中管理
- Star：30,198
- Fork：3,935
- 本期信号：+11,147 Star（trending）
- 能力：用统一入口连接不同模型服务；按配置把请求路由到目标模型；集中管理模型供应商和调用方式
