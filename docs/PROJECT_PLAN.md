...
> 决策原则：未得到用户确认的产品范围一律标记为 `TBD`；本文中的“默认建议”可用于继续开发，但不代表用户已经确认。

## 1. Executive Summary

### Problem Statement

人工发现 GitHub 热点、核对 Star 增量、整理中文亮点并维护日榜/周榜，存在耗时、口径不一致、数据不可追溯和内容生成失真的问题。当前仓库已经具备基础采集、快照、排名和报告能力，但仍缺少公开发布闭环、可浏览的项目页面、持续运营规范、稳定性监控和正式的人机审核流程。
...
As a 内容运营者, I want to 每天得到固定格式的 Top 3 榜单 so that 我能快速完成当天选题。

Acceptance Criteria:

- `python -m github_hotspots.cli run --period daily` 在有效配置下返回退出码 `0`。
- 产物包含 Markdown、JSON 和小红书文案，写入当日对应路径。
...
As a 内容运营者, I want to 每周得到 Top 7 或 Top 10 榜单 so that 我能制作覆盖面更广的周报。

Acceptance Criteria:

- `python -m github_hotspots.cli run --period weekly` 可独立运行。
- 默认输出 Top 7；仅修改配置即可切换 Top 10。
...
- `TBD`：需由用户确认是否采用“双榜”范围。
- 默认建议下，综合主榜不强制限定技术领域；AI 专题榜使用独立的来源查询、分类规则和 Top N 配置。
- 同一仓库可同时进入两榜，但页面和 JSON 必须显示其榜单范围及独立排名，不能复用排名序号。
- AI 分类依据必须可追溯到 topics、description、README 证据或人工规则，并允许维护排除名单。
...
- `TBD`：是否允许自动发布需用户明确确认；默认仅生成审核包。
- 审核包包含候选正文、标题、标签建议、数据更新时间、仓库链接和数据质量警告。
- 精确数字可回溯到当期 JSON/快照；修改后的文案不得覆盖原始数据证据。
- 人工审核状态至少区分 `draft`、`approved`、`rejected`；MVP 可先通过文件约定实现。
- 未标记 `approved` 的内容不得进入任何未来的发布适配器。
...
| Static site generator | 待建设 | 从已提交 JSON/Markdown 构建首页、历史页、方法页 | 构建失败保留上一版 Pages |
...
- 报告 JSON 增加稳定的 `scope` 字段；建议值为 `general` 和 `ai`，是否启用 AI 专题为 `TBD`。
- 综合主榜和专题榜共享事实快照，但分别配置候选规则、Top N 和排名结果。
- 迁移专题目录时保持现有 `reports/daily/YYYY-MM-DD.*`、`reports/weekly/YYYY-Www.*` 可读，避免破坏旧链接。
...
- 自动发布为 `TBD`，默认不实现；任何未来集成必须使用平台允许的接口或人工操作，不规避访问控制。
...
- 热点范围暂按综合主榜运行；“综合主榜 + AI 专题榜”为默认建议，待用户确认后进入配置开发。
- 小红书默认只生成文案审核包，人工发布。
...
- 在用户确认后增加综合榜/AI 专题榜独立配置、报告和 Pages 导航。
...
- 小红书或其他平台自动发布仅在用户明确确认、平台规则允许、具备审核门禁和撤回机制后实现。
...
| --- | --- | --- | --- | --- |
| M0 | 公开仓库初始化与首次推送 | GitHub 登录、仓库名确认 | `origin` 可访问，`main` 与本地一致 | 待执行 |
| M1 | 仓库级文档与运营规范 | M0 可并行起草 | README、PROJECT_LOG、PROJECT_PLAN、OPERATIONS 齐全 | 进行中 |
| M2 | Actions 首次线上验证 | M0、仓库 Actions 写权限 | daily/weekly 手动运行成功并产生可追踪产物 | 待执行 |
| M3 | 最小 Pages 上线 | M0、站点方案确认 | 公网 URL 可访问最新日/周榜及方法说明 | 待执行 |
| M4 | 7 日快照基线验收 | M2、连续运行 | 至少 7 个有效日期快照，周增量可复算 | 待积累 |
| M5 | 双榜范围实现 | 用户确认热点范围、M4 | 综合榜/AI 榜独立配置、测试、页面与报告 | `TBD` |
...
| M10 | 受控平台分发 | 用户明确批准、平台允许接口、M6 | 仅 approved 内容发布，可审计、可停止 | `TBD` / v2.0 |
...
1. 用户确认公开仓库名称和热点范围；未确认时采用 `github-hotspots` 与“综合主榜先行”的安全默认值。
2. 创建公开仓库、设置远程并完成首次推送，验证远程 `main`。
3. 按 `docs/OPERATIONS.md` 完成首次变更日志、测试、提交和发布说明。
4. 手动触发 daily/weekly Actions，确认写权限、产物提交和并发策略。
5. 上线最小 GitHub Pages，并记录实际 URL、部署方式和验证结果。
6. 连续积累 7 日快照；达到基线后核验周增量与榜单解释。
7. 单独验证本地 Codex 是否存在受支持的非交互调用方式；不读取认证文件，不让此项阻塞 MVP。
8. 用户审核本规划中的 `TBD` 决策，再锁定 v1.1 的双榜、图片和审核队列范围。
