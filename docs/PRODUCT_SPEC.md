...
- 每个榜单生成 1 张封面、每个入榜项目生成 1 张配置声明的合法 3:4 PNG（默认 `1200×1600`），数字与对应报告 JSON 一致率为 100%。
...
Signal Broadsheet 封面与逐项目海报
        ↓
publish/current 最新完整工作台 + publish/history 每期生成基线 → 人工审核发布
```
...
- Actions 日报在 09:17、周报在周日 10:27 兜底；同日期 Codex 报告和 publication history 均完整时跳过，只有 history 缺失时只修复 history。
- 本地任务使用独立 worktree、共享锁、严格 Codex 无回退门禁、路径白名单、Secret 扫描、安全 push 和 Pages 验证；两个 Actions 仍支持 `workflow_dispatch`。
- 只提交 `data/snapshots/`、对应报告目录和 `publish/history` 白名单文本；没有变更时不创建空提交。
...
- 综合主榜和 AI 专题榜分别生成 1 张封面；每个入榜项目分别生成 1 张 Manifest 声明的合法 3:4 PNG，默认 `1200×1600`。
...
- 本地 `publish/current` 将日报/周报的综合榜与 AI 榜拆成四个独立帖子包；`publish/history` 保存每期生成基线且不重复复制 PNG。2026-07-12 为 `D001` 和 `W001`，首发日前只能标记预览。
...
| 海报渲染器（V4） | 为每榜生成封面和逐项目合法 3:4 PNG（默认 `1200×1600`），展示 Owner 头像、最多 5 条能力、核心亮点与适合谁 | 头像失败使用占位图；缺字体、文字溢出、事实不一致或保存失败时明确失败 |
...
- 已增加每榜封面和逐项目可配置 3:4 V4 PNG（默认 `1200×1600`），采用参考图信息架构、安全 Owner 头像缓存和确定性占位图。
...
- [x] 每榜封面与逐项目 Manifest 尺寸 PNG 的数量、3:4 尺寸、中文字体和事实字段全部通过门禁。
