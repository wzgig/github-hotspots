...
构建一个 Python 3.12 项目，从 GitHub Trending、GitHub Search 和 GitHub API 获取候选与事实，按日/周保存快照，计算真实增量与热度分，再生成日榜 Top 3、周榜 Top 7（可配置 Top 10）的 Markdown 和小红书卡片文案。AI 只负责基于已提供证据压缩中文摘要，不生成或修改事实数字。

### 成功标准

- 连续 30 次非外部故障运行中，定时任务成功率不低于 95%。
- 日榜固定输出 Top 3；周榜默认输出 Top 7，并能仅修改配置切换为 Top 10。
...
作为内容运营者，我希望运行一个命令生成当天的 3 个热点项目，以便快速准备日更内容。

验收条件：

- 命令为 `python -m github_hotspots.cli run --period daily`。
- 配置从 `config/hotspots.yaml` 读取，默认 `top_n: 3`、`lookback_days: 1`。
- 报告写入 `reports/daily/`，并包含统计窗口、数据质量、Top 3 和方法说明。
- 每个有效项目包含项目名、定位、语言、总 Star、本期新增 Star 或缺失说明、Fork、一句话价值、3 个亮点、适用人群和仓库链接。

#### 故事 B：生成周榜

作为内容运营者，我希望每周生成覆盖面更广的榜单，并可在 7 个或 10 个项目之间切换。

验收条件：

- 命令为 `python -m github_hotspots.cli run --period weekly`。
- 默认 `top_n: 7`、`lookback_days: 7`，将 `top_n` 改为 10 后无需改代码即可输出 Top 10。
- 报告写入 `reports/weekly/`。
...
- 日榜 Top 3、周榜 Top 7，以及周榜 Top 10 配置能力。
- Markdown 总榜、小红书文本版卡片、方法与数据质量说明。
...
| 排名器 | 归一化各信号并计算可解释分数 | 保留各分量百分位，确保稳定排序 |
...
- 提供日榜 Top 3、周榜 Top 7；验证配置改为 Top 10 后输出正确。
- 通过测试并留下可复现命令。
...
- [ ] 快照可重读，增量可由两个快照独立复算。
- [ ] 日榜 Top 3、周榜 Top 7 和配置后的 Top 10 数量正确。
