...
构建一个 Python 3.12 的开源情报与内容生产项目，从 GitHub Trending、GitHub Search 和 GitHub API 获取候选与事实，按日/周保存快照并计算真实增量。系统对全部合格候选生成“综合主榜”，对可解释匹配的 AI 候选生成“AI 专题榜”，两榜独立排名且允许同一仓库同时入榜；日榜各 Top 3、周榜各 Top 7。系统输出包含双榜的 Markdown/JSON、两份小红书卡片文案，并以确定性代码渲染每榜封面和 V3 逐项目海报。默认使用确定性文案；可选本地 Codex Prompt/Schema 4.0 在清洗后的 README/metadata 证据范围内进行整榜白话编辑，身份、URL、排名、数字和许可证事实由程序冻结回查。
...
- Prompt/Schema 4.0 的逐字段证据 ID 合法率、README SHA 一致率、许可证门禁通过率和冻结事实一致率均为 100%，无证据新增事实率为 0%。
...
清洗 README / metadata，安全缓存 Owner 头像
        ↓
确定性兜底与 7 个候选 / 可选 Codex 4.0 证据编辑
        ↓
...
- V3 项目卡固定采用“系列顶部—项目身份—四项统计—最多 5 条能力—核心亮点—适合谁—页脚日期”的参考图信息架构，不复制参考帖原图、账号标识或文案。
...
- 确定性文案为默认路径；本地 Codex Prompt/Schema 4.0 只在同仓库 README/metadata 证据范围内进行白话编辑，任一输出校验失败时整榜回退。
- 单元测试、格式/静态检查和本地运行说明。
...
| 文案编辑 | 生成确定性兜底与 7 个候选；可选 Codex Prompt/Schema 4.0 在证据内白话改写 | 任一 Schema、证据、README SHA、许可证或事实校验失败时整榜使用确定性结果 |
| 渲染器 | 生成 Markdown 和小红书文本 | 模板缺字段时失败，不静默吞错 |
| 海报渲染器（V3） | 为每榜生成封面和逐项目 `1200×1600` PNG，展示 Owner 头像、最多 5 条能力、核心亮点与适合谁 | 头像失败使用占位图；缺字体、文字溢出、事实不一致或保存失败时明确失败 |
...
- 已实现 Prompt/Schema 4.0：程序清洗 README/metadata，Codex 可在逐字段证据边界内写白话内容，确定性兜底仍保持默认可用。
- 已增加每榜封面和逐项目 `1200×1600` V3 PNG，采用参考图信息架构、安全 Owner 头像缓存和确定性占位图。
- 已把 Schema 2 图片清单接入报告、Pages 预览/下载和日周工作流，并完成中文字体与字形门禁；跨系统 PNG 字节不作为一致性承诺。
- 已增加本地 `codex exec` Prompt/Schema 4.0 证据编辑适配器，并覆盖 README 缺失、非法证据 ID、README SHA/许可证/事实漂移和整榜回退；默认 CI 仍不启用。
...
- [x] Codex Prompt/Schema 4.0 输出通过逐字段证据、README SHA、许可证和冻结事实校验；README 缺失时只允许同一受控候选，失败时整榜回退。
- [x] Owner 头像经过安全缓存和重新编码，失败时 V3 海报使用确定性占位图。
