...
→ 综合候选/AI 分类 → 两榜独立排名 → 七类确定性候选/可选本地 Codex 受控选稿
...
- 默认先用确定性摘要器为整榜分配不同叙事角度，周榜至少覆盖 7 种开场模式，禁止反复套用“近期升温”“聚焦”“值得关注”“赋能”等模板句。
- 强定位与受众规则只能依据精确 Topics；自由描述属于不可信事实文本，不得通过简单关键词匹配生成能力结论。Topics 不足时使用中性回退。
...
- 每个榜单生成 1 张原创封面，每个项目生成 1 张 `1200×1600` PNG；数字和文字直接来自通过校验的报告对象，不抓取第三方图片，不使用随机在线素材。
...
7. 对摘要结果验证 JSON 可解析、字段类型、3 条亮点、数字/URL 原样复制、候选逐字符匹配、角度覆盖和证据映射。
8. 验证主 JSON 的顶层 `repositories` 兼容性、`boards` 双榜结构和两份小红书草稿路径。
9. 验证每榜封面与逐项目 PNG 数量、`1200×1600` 尺寸、`manifest.json`、无网络渲染、长名称/文本溢出和 Pages 图片复制路径。
...
- 默认确定性文案不含禁用套话且榜内角度多样；本地 Codex 仅作受控选稿，候选逐字符门禁通过并能安全回退。
- 卡片规定字段齐全，摘要无未支持事实。
