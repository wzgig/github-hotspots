...
- 五层更新方式：本地 Codex 每天 07:30 生成日报、周日 08:45 生成周报；根目录手动启动文件可检查并补更当期报告；Actions 在 09:17 / 周日 10:27 提供无本机凭据的确定性兜底；也可通过 Actions 或 CLI 按需运行。
...
| 方式 | 当前能力 | 适合场景 |
| --- | --- | --- |
| 本地日报主链路 | 每天北京时间 07:30，并在用户登录时执行缺失补跑，使用当前用户的 Codex CLI | 生成 README 证据化日报；若 Actions 已先生成完整兜底，则冻结排名事实后升级为正式 Codex 日报 |
| 本地周报主链路 | 每周日北京时间 08:45，使用当前用户的 Codex CLI；延迟到工作日启动时只恢复最近周日的冻结报告 | 生成周报，或把完整 deterministic 兜底安全升级为正式 Codex 周报 |
...
| 本地手动检查补更 | 双击 `CHECK_AND_UPDATE_REPORTS.cmd`；每天检查当日日报和最近一期应到周报 | 当日日报或应到周报已有完整 deterministic 兜底时冻结升级，不重新采集或重排事实 |
...
需要立即确认日报/周报是否已经同步时，可在仓库根目录双击 [CHECK_AND_UPDATE_REPORTS.cmd](CHECK_AND_UPDATE_REPORTS.cmd)。它复用与计划任务相同的严格事务：每次检查当日日报和最近一期应到周报；远端已有完整 Codex 报告和发布历史时幂等复用，只同步本地发布工作台并核验 Pages。若当日日报或应到周报已由 Actions 生成结构完整的 deterministic 兜底，本地任务会基于冻结报告刷新证据并重绘为 Codex 正式版，不重新采集或重排排名事实。北京时间周日发现当期完全缺失时可立即生成、提交并推送；周一至周六不会重新采集历史周榜。重绘前后自动比较双榜 `full_name/rank/stars/forks/star_delta/fork_delta/delta_source/html_url/score`；工作日周报完全缺失或损坏时仍返回退出码 76。
