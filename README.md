...
| 本地手动检查补更 | 双击 `CHECK_AND_UPDATE_REPORTS.cmd`；每天检查当日日报和最近一期应到周报 | 周日缺失时补更；工作日验证最近周报但不伪造历史数据 |
...
需要立即确认日报/周报是否已经同步时，可在仓库根目录双击 [CHECK_AND_UPDATE_REPORTS.cmd](CHECK_AND_UPDATE_REPORTS.cmd)。它复用与计划任务相同的严格事务：每次检查当日日报和最近一期应到周报；远端已有完整 Codex 报告和发布历史时幂等复用，只同步本地发布工作台并核验 Pages。北京时间周日发现当期缺失时可立即生成、提交并推送；周一至周六对最近周报使用只检查模式，缺失或降级时明确返回错误，但不会用当前累计数字伪造历史周报。
