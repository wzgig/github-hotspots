...
| 本地手动检查补更 | 双击 `CHECK_AND_UPDATE_REPORTS.cmd`；每天检查日报，周日同时检查周报 | 定时任务中断后的即时检查、补更、推送与 Pages 核验 |
...
需要立即确认日报/周报是否已经同步时，可在仓库根目录双击 [CHECK_AND_UPDATE_REPORTS.cmd](CHECK_AND_UPDATE_REPORTS.cmd)。它复用与计划任务相同的严格事务：远端已有完整 Codex 报告和发布历史时幂等跳过生成，只同步本地发布工作台并核验 Pages；当期缺失时立即生成、提交并推送。日报每天检查，周报只在北京时间周日到期并补更；其他日期只提示最近和下一次周日，不会用当前累计数字伪造历史周报。
