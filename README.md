...
| 本地手动检查补更 | 双击 `CHECK_AND_UPDATE_REPORTS.cmd`；每天检查当日日报和最近一期应到周报 | 周日缺失时补更；工作日自动升级已有冻结兜底，但不重新采集历史数据 |
...
需要立即确认日报/周报是否已经同步时，可在仓库根目录双击 [CHECK_AND_UPDATE_REPORTS.cmd](CHECK_AND_UPDATE_REPORTS.cmd)。它复用与计划任务相同的严格事务：每次检查当日日报和最近一期应到周报；远端已有完整 Codex 报告和发布历史时幂等复用，只同步本地发布工作台并核验 Pages。北京时间周日发现当期缺失时可立即生成、提交并推送；周一至周六不会重新采集历史排名，但如果最近周日已有结构完整的 deterministic 兜底，会基于该冻结报告刷新证据并重绘为 Codex 正式版。重绘前后自动比较双榜 `full_name/rank/stars/forks/star_delta/fork_delta/delta_source/html_url/score`；完全缺失或损坏时仍返回退出码 76。
